...
export default class PullRequestPatchContainer extends React.Component {
  ...
  buildPatch(rawDiff) {
    const { filter: filterDiff } = requireFromGitPanel("models/patch/filter");
    const { buildMultiFilePatch } = requireFromGitPanel("models/patch");
    ...
    const diffs = parseDiff(filtered).map((diff) => {
      ...
      return {
        ...
        newPath: diff.newPath
          ? toNativePathSep(diff.newPath.replace(/^[a|b]\//, ""))
          : diff.newPath,
        oldPath: diff.oldPath
          ? toNativePathSep(diff.oldPath.replace(/^[a|b]\//, ""))
          : diff.oldPath,
      };
    });
    const options = {
    ...
    return buildMultiFilePatch(diffs, options);
  }
