...
export default class StubItem {
  ...
  constructor(name, props = {}, uri) {
    ...
    this.element.classList.add(`github-panel-StubItem-${name}`);
    this.realItem = null;
  ...
  setRealItem(item) {
    ...
    if (item.onDidChangeTitle) {
      this.subscriptions.add(
        item.onDidChangeTitle((...args) => this.emitter.emit("did-change-title", ...args)),
      );
    }

    if (item.onDidChangeIcon) {
      this.subscriptions.add(
        item.onDidChangeIcon((...args) => this.emitter.emit("did-change-icon", ...args)),
      );
    }
